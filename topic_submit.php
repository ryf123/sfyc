<?php
	...
	class topic_submit{
		...
		private function insert_photo($photo){	
			...
			$query = "insert into ".PHOTO_TABLE." values('".$photo."','".$this->topic_id."','".$photo_id."','".$this->username ."',CURRENT_TIMESTAMP)";
			echo $query; 
		...
		private function insert_topic($topic){
			$query = "insert into ".TOPIC_TABLE." values('".$this->topic_id."','".$this->username."','".$topic."',CURRENT_TIMESTAMP)";
			echo $query; 
			if($this->mysql->query($query)){
				return true;
			}
